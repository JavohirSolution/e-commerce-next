...
const ShoppingCard = () => {
  ...
  const removeProduct = (id: number) => {
    const upadtedCarts = products.filter(product => product.id !== id)
    localStorage.setItem("carts", JSON.stringify(upadtedCarts));
    setProducts(upadtedCarts)
  }

  const handleIncrement = (id: number) => {
    const updatedCarts = products.map(product => {
      if (product.id === id) {
        return {
          ...product,
        ...
        <div className="h-screen bg-gray-100 pt-20">
          ...
          <div className="mx-auto max-w-5xl justify-center px-6 md:flex md:space-x-6 xl:px-0">
                  ...
                  <div className="sm:ml-4 sm:flex sm:w-full sm:justify-between">
                    <div className="mt-5 sm:mt-0">
                      <div>
                        ...
                        <p className="text-xs font-medium line-clamp-2 mt-0">{product.description}</p>
                      </div>
                      <p className="mt-1 text-xs text-gray-700 flex gap-x-1">
                        <ReactStars
                    ...
                    <div className="mt-4 flex justify-between sm:space-y-6 sm:mt-0 sm:block sm:space-x-6">
                      <div className="flex items-center border-gray-100">
                        ...
                        <input className="h-8 w-8 border bg-white text-center text-xs outline-none" value={product.quantity} type="number" min="1" />
                      ...
                      <div className="flex items-center">
                        ...
                        <svg
                          ...
                          className="h-5 w-5 cursor-pointer duration-150 text-red-500 hover:scale-110">
            ...
            <div className="mt-6 h-full rounded-lg border bg-white p-6 shadow-md md:mt-0 md:w-1/3">
              ...
              <button
                className="mt-6 w-full rounded-md bg-sky-600 py-4 font-medium text-blue-50  
            hover:bg-transparent hover:text-black border
          hover:border-sky-500 transition duration-200 ease">
                Check out
              </button>
            </div>
          </div>
